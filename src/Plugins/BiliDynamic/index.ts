...
class BiliDynamicSubscriber extends BiliSubscriber {
	...
	private parseDynamicCardToString(cardStr: string, item: any = null): string {
		if (!cardStr) {
		...
		if (card.aid) {
			...
			// TODO 投稿视频（？）
			return (
				"发了视频 " + `${card.dynamic ? card.dynamic + "\n" : ""} ` + videoText
			);
		} else if (card.origin) {
			// TODO 转发动态（？）
			// origin.item
			...
			if (itm.pictures) {
				...
				if (card.summary) {
					...
					if (card.origin_image_urls && card.origin_image_urls.length) {
	...
	async runHandler() {
		...
		if (rec.latest_dynamic_id == info.dynamic_id) {
			...
			try {
				...
				log.debug(data);
				const recs = await DBHandler.getBiliRec<BiliDynamicType.dynamicRec>(
					this.tableName,
					rec.uid,
					info.dynamic_id
				);
				recs.forEach((dynamic) => {
					qq.sendToGroupSync(dynamic.group_id, [
